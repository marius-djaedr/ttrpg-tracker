...
//////////////////////////////////////////////////////////////////////////
    ...
    function buildDataFunc_cumulative(playMap, runMap){
        let ran = 0.0;
        let played = 0.0;

        buildDataFunc_common(playMap, runMap, (playValue, runValue) =>{
            ran += runValue;
            return ran;
        }, (playValue, runValue) =>{
            played += playValue;
            ...
            return ran<0.001 ? null : played/ran;
        })
    }



    function buildDataFunc_common(playMap, runMap, col2Func, col3Func, col4Func){
        const playYears = [...Object.keys(playMap)];
        const runYears = [...Object.keys(runMap)];
        ...
        for(let y = firstSessionYear; y<currentYear; y++){
            ...
            for(let m = 0; m<12; m++){
                ...
                for(let d=1; d <= monthDayArray[m]; d++){
                    playValue = playMap_month[d] == null? 0.0 : playMap_month[d];
                    runValue = runMap_month[d] == null? 0.0 : runMap_month[d];

                    let col2 = col2Func(playValue, runValue);
                    let col3 = col3Func(playValue, runValue);
                    let col4 = col4Func(playValue, runValue);

                    data.push([AggUtils.buildDateFunctionString(new Date(y, m, d)), col2, col3, col4]);
                }
            }
        }
        ...
        for(let m = 0; m<=currentMonth; m++){
            ...
            for(let d=1; d <= monthDayArray[m]; d++){
                ...
                data.push([AggUtils.buildDateFunctionString(new Date(currentYear, m, d)), col2, col3, col4]);
            }
        }
